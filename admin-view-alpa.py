...
grade_label.grid(row=4,column=0)
        ...
        ##BUTTONS 
add_student_btn=Button(root2, text="add student record", command=add_newStudent)
add_student_btn.grid(row=0, column=3)

        
addGrades_btn=Button(root2, text="view grades",command=view_grades_A)
addGrades_btn.grid(row=1, column=3)

searchR_btn=Button(root2, text="search records", command=search_student,)
searchR_btn.grid(row=2, column=3 )

census_btn=Button(root2, text="print class census", command=census)
census_btn.grid(row=3, column=3)

addG_btn= Button(root2, text="add grades", command=add_grades2)
addG_btn.grid(row=4, column=3)

# create the listbox (height/width in char)
listbox = Listbox(root2, width=30, height=10,)
